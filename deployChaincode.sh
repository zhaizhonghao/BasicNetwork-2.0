...
getBlock() {
    setGlobalsForPeer0Org1
    # peer channel fetch 10 -c basicchannel -o localhost:7050 \
    #     --ordererTLSHostnameOverride orderer.example.com --tls \
    #     --cafile $ORDERER_CA

    peer channel getinfo  -c basicchannel -o localhost:7050 \
        --ordererTLSHostnameOverride orderer1.example.com --tls \
        --cafile $ORDERER_CA
}
...
chaincodeQuery() {
    ...
    # Query Car by Id
    peer chaincode query -C $CHANNEL_NAME -n ${CC_NAME} -c '{"function": "queryCar","Args":["Car-ABCDEEE"]}'
